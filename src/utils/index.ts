...
export function initTime(timestamp:number | string | bigint){
    if(!timestamp && timestamp !== 0)return '--'
    
    // 将时间戳转换为数字
    let timeNum: number
    if (typeof timestamp === 'bigint') {
        timeNum = Number(timestamp.toString())
    } else if (typeof timestamp === 'string') {
        timeNum = Number(timestamp)
    } else {
        timeNum = timestamp
    }
    
    // 判断是秒级时间戳还是毫秒级时间戳（10位是秒，13位是毫秒）
    if (timeNum.toString().length <= 10) {
        timeNum = timeNum * 1000 // 转换为毫秒
    }
    
    const date = new Date(timeNum); // 将时间戳转换为 Date 对象
